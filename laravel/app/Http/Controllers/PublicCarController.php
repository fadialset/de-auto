    ...
    public function addToMyCars($id)
{
    ...
    if (!auth()->check()) {
        return redirect()->route('login')
            ->withErrors(['error' => 'You need to log in to add cars.']);
    }
    ...
    if ($carExists) {
        return redirect()->back()
            ->with('error', 'You have already added this car to your collection.');
    }

    // Create the car for the authenticated user using the relationship
    ...
    return redirect()->route('public-cars.index')
        ->with('success', 'Car added to your collection!');
}
}
